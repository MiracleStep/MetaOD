...
from . import env_google_vision_api as EG
from . import env_amazon_api as EA
from . import env_ibm_api as EI
from . import env_microsoft_api as MA
# import env_amazon_api as EA
from . import env_yolo_local_api as EY
...
class ENV(object):
    ...
    def __init__(self, bg, ob, use_raw_pixel, agent=(0, 0), target=None):
        ...
        self.init_configs() # 目标检测背景图片并根据检测结果初始化一些数据。
        self.reset()
    ...
    def init_configs(self):
        ...
        res = EY.localize_objects(self.bg)
        print(res)
        # res is the ground truth
        # 2019-06-22 let's store the ground truth
        with open("0001.txt", 'w') as f: # 将预测的结果写入文件中(可以看做真实结果)，后续进行mAP计算。
            for r in res:
                f.write(r[0] + " " + " " + str(r[2][0][0]) + " " + str(r[2][0][1]) + " " + str(r[2][1][0]) + " " + str(
                    r[2][1][1]) + "\n")
    ...
    # 计算预测结果的mAP值，如果不为100%，则return True表示模型检测出现问题。（首先排除插入目标的边界框）
    def compute_terminate_new(self, x, y, res):
        print("check error:")
        # compute upper left corner coordinate and therefore rule out inserted object
        # 计算插入对象的左上角坐标，从而排除插入的对象
        x0 = int(x - self.obj_nrows / 2)
        y0 = int(y - self.obj_ncols / 2)

        al = []
        for r in res: # 判断插入后图像的每个检测框结果
            x1 = r[2][0][0]
            y1 = r[2][1][0]
            x2 = r[2][0][1]
            y2 = r[2][1][1]
            xc = (x1 + x2) / 2 # 检测框中心点xc, yc
            yc = (y1 + y2) / 2
            # 排序插入对象预测的边界框
            if abs(x1 - x0) < 10 and abs(y1 - y0) < 10: # 预测的边界框与插入目标的边界框相近
                # the inserted object so we remove it
                print("REMOVE")
                continue
            elif self.check_overlapping_inserted_box(xc, yc, x, y):# 比较预测框中心点坐标和插入目标的中心点是否重叠
        ...
        if len(res) != 0:
            with open("0001.txt", 'w') as f: # 将预测结果写入文件中，并进行mAP计算。
                for r in res:
                    f.write(
                        r[0] + " " + str(r[1] / 100)[0:3] + " " + str(r[2][0][0]) + " " + str(r[2][0][1]) + " " + str(
                            r[2][1][0]) + " " + str(r[2][1][1]) + "\n")
    ...
    def get_reward(self, idx):
        ...
        if res0 == None:
            ...
            res = EY.localize_objects(self.new_path)
            # print (res)
    ...
    def check_overlapping(self, row, col, a):
        ...
        box0 = (box_row, box_col) # 插入对象的边界框框的位置：((x0, x1), (y0, y1))
        for box in self.object_boxes: # 与背景图像被插入对象的边界框位置进行比较。
            if overlapping2D(box0, box):
                return True
        return False

    # 比较预测框中心点坐标和插入目标的中心点是否重叠
    ...
    def do_insert(self, x, y, idx):
        res0 = self.cache_insertion_results(x, y) # 检查x,y坐标是否已经插入过了，插入过就返回插入目标后图像的目标检测结果(缓存)，否则为None
        if res0 == None: # 没有插入过,没有缓存
...
if __name__ == "__main__":
    ...
    env = ENV(bg, ob, use_raw_pixel=True)
