...
def get_guided_insertion_location(arr):
    def compute_distance(target_obj_x, target_obj_y, inserted_obj_x, inserted_obj_y): # 计算距离：blue region：x1蓝色框内框的宽度  x2蓝色框外框宽度...
...
def process():
    _ = env.reset()
    a = collect_regions_of_objects(env.object_boxes) # 统计背景图像上目标检测box的：[(delta_box, 中心点，宽， 高),..] [(delta_box, 中心点，宽， 高),..]
    win_list = []
    # FIXME
...
if __name__ == '__main__':  # 测试用
    main("000000003469.jpg", "remote_000000000188_2.png", 51, 76, 1)
